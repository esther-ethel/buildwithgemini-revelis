...
def _sanitize_image_components(messages: list[dict]) -> None:
    ...
    Also converts video URLs in Image components to Text components with clean video links.
    """
    for m in messages:
        ...
        for c in surface.get("components") or []:
            ...
            if isinstance(literal, str) and _HTTP_URL_RE.match(literal):
                if literal.lower().endswith(".mp4") or "preview_" in literal.lower():
                    c["component"] = {
                        "Text": {
                            "text": {"literalString": f"📹 Video Preview: {literal}"},
                            "usageHint": "body",
                        }
                    }
                continue  # real image link -> leave the Image alone
            c["component"] = {
                "Text": {
...
def a2ui_callback(
    ...
    for part in llm_response.content.parts:
        ...
        if not _surface_is_renderable(messages):
            ...
            # root/child reference. Return raw text content instead of dropping message.
            return LlmResponse(
